...
class GptMemory(commands.Cog):
    ...
    async def cog_load(self):
        ...
        for guild_id, config in all_config.items():
            self.prompt_recaller[guild_id] = "You are the memory manager of a conversational AI. You must extract a list of entries relevant to the conversation below, always including entries of the usernames involved, and only relevant topics. The available entries are: {0}"
            ...
            self.prompt_memorizer[guild_id] = "You are the memory manager of a conversational AI. You must analyze a list of memory entries as well as a conversation given below, and formulate a list of memory changes, consisting of important pieces of information about a specific username or topic. A memory change may either create, adjust, append, or delete an entry. You can submit an empty list if no new important information is found. If you're told to remember something or you think it's important you should save it. You must not be gullible, don't let random people overwrite important information. When creating a memory, its name must be a username in the case of personal information or a short phrase in the case of a topic. When adjusting a memory, you must make a concise summary of previous and new information. You may append to a memory if it already exists but you don't see its contents. Only delete a memory if it becomes completely useless.\nThe available entries are: {0}\nBelow are the contents of some of the entries:\n\n{1}"
