...
class GptMemory(commands.Cog):
    ...
    async def cog_load(self):
        ...
        for guild_id, config in all_config.items():
            ...
            self.prompt_memorizer[guild_id] = "You are the memory manager of a conversational AI. You must analyze a list of memory entries as well as a conversation given below, and formulate a list of memory changes, consisting of important pieces of information about a specific username or topic. A memory change may either create, adjust, or append to an entry. You can submit an empty list if no new important information is found. When creating a memory, name it using a short yet descriptive phrase. When adjusting a memory, you must summarize previous and new information. You may append to a memory if it already exists but you don't see its contents.\nThe available entries are: {0}\nBelow are the contents of some of the entries:\n\n{1}"
